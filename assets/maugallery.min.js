...
$(this).fadeIn(500)})};$.fn.mauGallery.defaults={columns:3,lightBox:!0,lightboxId:null,showTags:!0,tagsPosition:"bottom",navigation:!0};$.fn.mauGallery.listeners=function(options){$(".gallery-item").on("click keydown",function(event){if(event.type==="click"||event.key==="Enter"){if(options.lightBox&&$(this).prop("tagName")==="IMG"){$.fn.mauGallery.methods.openLightBox($(this),options.lightboxId)}else{return}}});$(".gallery").on("click keydown",".nav-link",function(event){if(event.key==="Enter"||event.type==="click"){$.fn.mauGallery.methods.filterByTag.call(this,event)}});$(".gallery").on("click keydown",".mg-prev",function(event){if(event.key==="Enter"||event.type==="click"){$.fn.mauGallery.methods.prevImage(options.lightboxId)}});$(".gallery").on("click keydown",".mg-next",function(event){if(event.key==="Enter"||event.type==="click"){$.fn.mauGallery.methods.nextImage(options.lightboxId)}})};$.fn.mauGallery.methods={createRowWrapper(element){if(!element.children().first().hasClass("row")){element.append('<div class="gallery-items-row row"></div>')}},wrapItemInColumn(element,columns){if(columns.constructor===Number){element.wrap(`<div class='item-column mb-4 col-${Math.ceil(12 / columns)}'></div>`)}else if(columns.constructor===Object){var columnClasses="";if(columns.xs){columnClasses+=` col-${Math.ceil(12 / columns.xs)}`}
...
let index=0,next=null;$(imagesCollection).each(function(i){if($(activeImage).attr("src")===$(this).attr("src")){index=i}});next=imagesCollection[index+1]||imagesCollection[0];$(".lightboxImage").attr("src",$(next).attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
        lightboxId ? lightboxId : "galleryLightbox"
            ...
            </div>`)},showItemTags(gallery,position,tags){var tagItems='<li class="nav-item"><span class="nav-link active active-tag"  data-images-toggle="all" tabindex=0>Tous</span></li>';$.each(tags,function(index,value){tagItems+=`<li class="nav-item active">
                <span class="nav-link"  data-images-toggle="${value}" tabindex=0>${value}</span></li>`});var tagsRow=`<ul class="my-4 tags-bar nav nav-pills">${tagItems}</ul>`;if(position==="bottom"){gallery.append(tagsRow)}else if(position==="top"){gallery.prepend(tagsRow)}else{console.error(`Unknown tags position: ${position}`)}},filterByTag(){if($(this).hasClass("active-tag")){return}
